 ...
 * La URL base se lee de ENV: VITE_TRAINING_API_URL (.env o .env.local).
 */
...
async function get<T>(path: string, params?: Record<string, string | number | undefined>): Promise<T> {
  ...
  if (import.meta.env.DEV) console.log("[Training API] GET", url)
  const res = await fetch(url)
  const json = await res.json().catch(() => ({}))
  if (!res.ok || json?.ok === false) throw new Error(json?.error ?? `Error ${res.status}`)
  // Aceptar { data }, array directo, o { history/records } por compatibilidad con distintos backends
  const payload = json.data ?? (Array.isArray(json) ? json : json.history ?? json.records ?? undefined)
...
export async function getInvites(sessionId: number): Promise<SessionInvite[]> {
  return get<SessionInvite[]>("/invite", { session_id: sessionId })
}
